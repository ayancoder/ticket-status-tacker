...
export const loadUser = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`http://${process.env.SERVER}:5000/api/auth`);
...
export const register = (name, password, phone) => async (dispatch) => {
  try {
    ...
    const res = await axios.post(
      `http://${process.env.SERVER}:5000/api/users`,
      body,
      config
    );
...
export const login = (password, phone) => async (dispatch) => {
  try {
    ...
    const res = await axios.post(
      `http://${process.env.SERVER}:5000/api/auth`,
      body,
      config
    );
...
export const fetchAllDealingOfficer = () => async (dispatch) => {
  try {
    const res = await axios.get(`http://${process.env.SERVER}:5000/api/users`);
    console.log(res);
    if (res) {
      dispatch({
